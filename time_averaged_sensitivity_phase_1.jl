#This file is used to calculate the time averaged sensitivity matrix for phase 1. time unit 21 to 40.
...
dProt_j_by_dp_i_time_21 = rand(13,3)
dProt_j_by_dp_i_time_31 = rand(13,3)
dProt_j_by_dp_i_time_40 = rand(13,3)

#For time value 21
for i in 1:13          # 13 parameter values
    for j in 1:3       # P1, P2, P3
        dProt_j_by_dp_i_time_21[i,j] = (X_i_total[i][1,j] - X_original[21,j+3])/(dp[i])
    end
end
...
for i in 1:13          # 13 parameter values
    for j in 1:3       # P1, P2, P3
        dProt_j_by_dp_i_time_31[i,j] = (X_i_total[i][10,j] - X_original[31,j+3])/(dp[i])
    end
end
...
for i in 1:13          # 13 parameter values
    for j in 1:3       # P1, P2, P3
        dProt_j_by_dp_i_time_40[i,j] = (X_i_total[i][20,j] - X_original[40,j+3])/(dp[i])
    end
end
...
s_time_averaged_sensitivity_phase_1 = (dProt_j_by_dp_i_time_21+dProt_j_by_dp_i_time_31+dProt_j_by_dp_i_time_40)/3
