#This file is used to calculate the time averaged sensitivity matrix for early phase 2. time unit 71 to 90.
...
dProt_j_by_dp_i_time_71 = rand(13,3)
dProt_j_by_dp_i_time_81 = rand(13,3)
dProt_j_by_dp_i_time_90 = rand(13,3)

#For time value 71
for i in 1:13          # 13 parameter values
    for j in 1:3       # P1, P2, P3
        dProt_j_by_dp_i_time_71[i,j] = (X_i_total[i][1,j] - X_original[71,j+3])/(dp[i])
    end
end
...
for i in 1:13          # 13 parameter values
    for j in 1:3       # P1, P2, P3
        dProt_j_by_dp_i_time_81[i,j] = (X_i_total[i][10,j] - X_original[81,j+3])/(dp[i])
    end
end
...
for i in 1:13          # 13 parameter values
    for j in 1:3       # P1, P2, P3
        dProt_j_by_dp_i_time_90[i,j] = (X_i_total[i][20,j] - X_original[90,j+3])/(dp[i])
    end
end
...
s_time_averaged_sensitivity_phase_2_early = (dProt_j_by_dp_i_time_71+dProt_j_by_dp_i_time_81+dProt_j_by_dp_i_time_90)/3
